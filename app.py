...
def render_header() -> None:
    ...
    logo_html = f'<img src="{logo_data_uri}" alt="GEA Chemical">' if logo_data_uri else ""
    st.markdown(
        f"""
        <div class="gea-header">
            {logo_html}
            <div>
                <h1 class="gea-title">{APP_TITLE}</h1>
                <p class="gea-subtitle">Consulta gerencial de inventario y despachos en tiempo casi real.</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_dispatch_tab(dispatch_df: pd.DataFrame) -> None:
    ...
    display_df["Total kg"] = display_df["Total kg"].map(format_kg)
    display_df = display_df.rename(columns={"Total kg": "Total kg"})
    st.dataframe(display_df, width="stretch", hide_index=True)
