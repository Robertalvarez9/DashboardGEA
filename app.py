...
REMOTE_LOGO_URL = "https://geachemical.com/wp-content/uploads/2025/01/logo-gea-chemical-280x106.png"
LOGO_PATHS = [
...
def get_logo_path() -> Path | None:
    for logo_path in LOGO_PATHS:
        if logo_path.exists():
            return logo_path
    for logo_path in DASHBOARD_DIR.rglob("*logo*"):
        if logo_path.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp"}:
            return logo_path
    return None


def current_lima_time() -> datetime:
    ...
    with st.container(border=True):
        ...
        with logo_col:
            if logo_path is not None:
                st.image(logo_path.read_bytes(), width=280)
            else:
                st.image(REMOTE_LOGO_URL, width=280)
        with title_col:
            st.title(APP_TITLE)
            st.caption("Consulta de inventario y despachos en tiempo real.")
...
def load_dashboard_users() -> dict[str, str]:
    ...
    try:
        ...
        configured_users = auth_config.get("users", {}) if hasattr(auth_config, "get") else {}
        users = {
            str(username).strip().casefold(): str(password)
            for username, password in dict(configured_users).items()
...
def check_password() -> bool:
    ...
    if not users:
        st.error("Falta configurar usuarios en .streamlit/secrets.toml.")
    ...
    with login_col:
        with st.container(border=True):
            ...
            with st.form("login_form"):
                ...
                remember_session = st.checkbox("Mantener la sesión iniciada")
                submitted = st.form_submit_button("Ingresar", use_container_width=True)

    if submitted:
        username_key = username.strip().casefold()
        expected_password = users.get(username_key, "")
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame, datetime]:
    ...
    if config_issue:
        ...
        return _empty_frame(INVENTORY_COLUMNS), _empty_frame(DISPATCH_COLUMNS), current_lima_time()
    ...
    return inventory_df, dispatch_df, current_lima_time()
